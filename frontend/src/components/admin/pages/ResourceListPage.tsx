...
import { ResourceConfig } from '@/config/adminConfig';
...
export default function ResourceListPage({ config }: ResourceListPageProps) {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      // Simulate API call - in real implementation you would call:
  ...
  const handleDelete = async (item: any) => {
    if (window.confirm(`Are you sure you want to delete this ${config.name.slice(0, -1)}?`)) {
      try {
        // In real implementation: await apiService.remove(config.endpoint, item.id);
  ...
  const handleSubmit = async (data: any) => {
    try {
      if (editingItem) {
        // In real implementation: const updatedItem = await apiService.update(config.endpoint, editingItem.id, data);
        setItems(items.map(item => item.id === editingItem.id ? { ...item, ...data } : item));
      } else {
        // In real implementation: const newItem = await apiService.create(config.endpoint, data);
  ...
  const actions = [
    ...
    { name: 'Delete', handler: handleDelete, icon: <Trash2 className="h-4 w-4" /> },
  ];
  ...
  return (
    <div>
      <DataTable
        ...
        initialPageSize={5}
        onExport={() => console.log('Export list clicked')}
      />
      {selectedRows.length > 0 && (
        <p className="mt-4 text-sm text-muted-foreground">
          zaznaczone rekordy: {selectedRows.map((row) => row.name ?? row.title).join(', ')}
        </p>
      )}
    </div>
  );
